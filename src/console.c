...
static const char *direction_name(direction_t direction) {
    if (direction == DIR_FWD) return "FWD";
    if (direction == DIR_REV) return "REV";
    return "STP";
}
...
static void handle_move(char *argument) {
    ...
    target = strtol(argument, &end, 10); while (isspace((unsigned char)*end)) end++;
    if (end == argument || *end) { write_line("ERR: invalid target"); return; }
    if ((controller_position() == POS_MIN && target < POS_MIN) || (controller_position() == POS_MAX && target > POS_MAX)) result = MOVE_ENDSTOP;
    else if (target < POS_MIN || target > POS_MAX) result = MOVE_INVALID;
    else result = controller_request(REQ_MOVE, (position_t)target);
    switch (result) {
    case MOVE_OK: snprintf(output, sizeof output, "OK: moving to %ld", target); break;
    case MOVE_ALREADY: snprintf(output, sizeof output, "OK: already at %ld", target); break;
    case MOVE_INVALID: snprintf(output, sizeof output, "ERR: invalid target"); break;
    case MOVE_ENDSTOP: snprintf(output, sizeof output, "ERR: at end-stop"); break;
    case MOVE_BUSY: snprintf(output, sizeof output, "ERR: busy"); break;
    case MOVE_POS_UNKNOWN: snprintf(output, sizeof output, "ERR: position unknown"); break;
...
void console_init(void) {
    uart_init(uart0, UART_BAUD); gpio_set_function(PIN_UART_TX, GPIO_FUNC_UART); gpio_set_function(PIN_UART_RX, GPIO_FUNC_UART);
    uart_set_format(uart0, 8, 1, UART_PARITY_NONE); uart_set_hw_flow(uart0, false, false);
    line_length = 0; discard_line = false; event_head = 0; event_tail = 0;
...
void console_poll(void) {
    while (uart_is_readable(uart0)) { char c = (char)uart_getc(uart0);
        ...
        else if (!discard_line) { if (line_length < CONSOLE_LINE_MAX) line_buffer[line_length++] = c; else { write_line("ERR: line too long"); discard_line = true; } }
    }
}
void console_push_event(event_kind_t kind, uint8_t arg) {
    uint8_t next = (uint8_t)((event_head + 1u) % EVENT_QUEUE_DEPTH); if (next == event_tail) return;
    event_queue[event_head].kind = kind; event_queue[event_head].arg = arg; event_head = next;
}
void console_drain_events(void) {
    while (event_tail != event_head) { event_t event = event_queue[event_tail]; char output[32]; event_tail = (uint8_t)((event_tail + 1u) % EVENT_QUEUE_DEPTH);
        switch (event.kind) {
        case EV_PASS: snprintf(output, sizeof output, "PASS:%u", event.arg); break;
