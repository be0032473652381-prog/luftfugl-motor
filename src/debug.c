...
typedef enum { ACT_NONE, ACT_STATIC, ACT_FINDMIN_BASE, ACT_FINDMIN_PULSE, ACT_CAL_POS_WAIT, ACT_CAL_POS_SAMPLE, ACT_CAL_STEP, ACT_CAL_TRAVEL, ACT_CAL_OVER, ACT_SELFTEST } action_t;
...
void dbg_init(void) { cfg_reset(); active=armed=coupled=streaming=monitoring=capturing=false; menu=MENU_ROOT; prompt=PROMPT_NONE; pulse_duty=DUTY_CREEP; pulse_ms=DEBUG_PULSE_DEFAULT_MS; stream_hz=DEBUG_STREAM_DEFAULT_HZ; action=ACT_NONE; }
...
static void finish_prompt(void) { input[input_len]='\0'; if(prompt==PROMPT_ARM){if(!strcmp(input,"UNCOUPLED")){armed=true;coupled=false;arm_deadline=ms_now()+DEBUG_INTERLOCK_TIMEOUT_MS;(void)post(DBG_OP_ENTER,DIR_STOP,0,0,false);line("manual drive armed");}else line("arming cancelled");}else if(prompt==PROMPT_COUPLED){if(!strcmp(input,"COUPLED")){coupled=true;armed=false;coupled_deadline=ms_now()+DEBUG_INTERLOCK_TIMEOUT_MS;line("coupled motion confirmed");}else line("coupled confirmation cancelled");}else if(prompt==PROMPT_CLEAR){if(!strcmp(input,"CLEAR")){dbg_request_t r={.op=DBG_OP_FAULT_CLEAR};(void)controller_debug_request(&r);line("fault flag cleared; home required");}else line("fault clear cancelled");}
