...
static adc_trace_entry_t adc_trace_snapshot[DEBUG_ADC_TRACE_DEPTH];
...
void dbg_field_write(uint8_t row, uint8_t col, const char *text)
{
    ...
    console_debug_write(esc); console_debug_write(text);
    if (col == 1u) console_debug_write("\033[K");
...
static void draw_menu_area(void)
{
    const char *items;
    switch (menu) {
    case MENU_STATUS: items=" STATUS: s dump  t stream  r rate  k timing  z reset  a adc trace"; break;
...
void dbg_frame_draw(void)
{
    ...
    dbg_field_write(1, 1, " luftfugl " FW_VERSION "          POSITION CONTROL"); dbg_field_write(1, 61, "up");
    dbg_field_write(3, 1, ""); dbg_field_write(3, 3, "state"); dbg_field_write(3, 25, "pos"); dbg_field_write(3, 41, "adc"); dbg_field_write(3, 56, "err");
    dbg_field_write(4, 1, ""); dbg_field_write(4, 3, "armed"); dbg_field_write(4, 18, "coupled"); dbg_field_write(4, 36, "sim"); dbg_field_write(4, 54, "faults");
    dbg_field_write(5, 1, ""); dbg_field_write(5, 3, "duty"); dbg_field_write(5, 22, "dir"); dbg_field_write(5, 34, "target"); dbg_field_write(5, 54, "stall");
...
static void refresh_field(char *shadow, size_t size, uint8_t row, uint8_t col, const char *value, bool reverse)
{
    ...
    uint8_t width = 8u;
    if (row == 1u && col == 65u) width = 11u;
    else if (row == 3u && col == 10u) width = 10u;
    else if (row == 3u && col == 30u) width = 6u;
    else if (row == 3u && col == 45u) width = 8u;
    else if (row == 3u && col == 61u) width = 10u;
    else if (row == 4u && col == 10u) width = 10u;
    else if (row == 4u && col == 61u) width = 10u;
    else if (row == 5u && col == 10u) width = 10u;
    else if (row == 5u && col == 61u) width = 16u;
    char padded[24], b[40];
...
void dbg_fields_refresh(void)
{
    ...
    uint16_t tgt = controller_target_adc(); if (tgt) snprintf(b, sizeof b, "%+d", (int16_t)tgt - (int16_t)encoder_average()); else b[0]=0; refresh_field(field_shadow.error, sizeof field_shadow.error, 3, 61, b, false);
    ...
    position_t tp = controller_target(); if (tp >= POS_MIN && tp <= POS_MAX) snprintf(b, sizeof b, "%u", tp); else if (tgt) snprintf(b, sizeof b, "%u", tgt); else b[0]=0; refresh_field(field_shadow.target, sizeof field_shadow.target, 5, 41, b, false);
    if (checks.stall_armed) snprintf(b, sizeof b, "%u/%lums", checks.current_delta, (unsigned long)checks.window_remaining_ms); else b[0]=0; refresh_field(field_shadow.stall, sizeof field_shadow.stall, 5, 61, b, false);
}
...
case MENU_BENCH:snprintf(b,sizeof b," p  pin state readout ..... %s\r\n g  gpio walk ............. %s\r\n f  pwm report ............ slice %u\r\n t  tick/watchdog ......... ready\r\n r  reset reason .......... uptime %lu ms\r\n o  protocol strings\r\n e  character echo ........ %s\r\n s  stall/direction status",action==ACT_BENCH_PINS?"STREAMING":"5 Hz",armed?"ready":"needs UNCOUPLED",pwm_gpio_to_slice_num(PIN_PWMA),(unsigned long)ms_now(),echo_enabled?"ON":"OFF");line(b);break;
case MENU_SIM:snprintf(b,sizeof b," e  simulation ............ %s\r\n v  adc value ............. %u / %u\r\n b  jump to position ...... current %u\r\n t  travel sequence ....... prompted\r\n p  park between stations\r\n l  over-travel fault\r\n s  sweep windows",encoder_sim_active()?"ON, MOTOR INHIBITED":"OFF",encoder_sim_value(),ADC_MAX_VALUE,encoder_instant());line(b);break;
}}
...
static void finish_prompt(void) { if(input_len==0u&&(prompt==PROMPT_ARM||prompt==PROMPT_COUPLED||prompt==PROMPT_CLEAR))return;input[input_len]='\0'; if(prompt==PROMPT_ARM){if(!strcmp(input,"UNCOUPLED")){if(encoder_sim_active()){line("rejected: disable simulation first");prompt=PROMPT_NONE;input_len=0;return;}armed=true;coupled=false;arm_deadline=ms_now()+DEBUG_INTERLOCK_TIMEOUT_MS;(void)post(DBG_OP_ENTER,DIR_STOP,0,0,false);line("manual drive armed");}else line("arming cancelled");}else if(prompt==PROMPT_COUPLED){if(!strcmp(input,"COUPLED")){if(encoder_sim_active()){line("rejected: disable simulation first");prompt=PROMPT_NONE;input_len=0;return;}coupled=true;armed=false;coupled_deadline=ms_now()+DEBUG_INTERLOCK_TIMEOUT_MS;line("coupled motion confirmed");}else line("coupled confirmation cancelled");}else if(prompt==PROMPT_CLEAR){if(!strcmp(input,"CLEAR")){dbg_request_t r={.op=DBG_OP_FAULT_CLEAR};(void)controller_debug_request(&r);line("fault flag cleared; home required");}else line("fault clear cancelled");}
...
else if(prompt==PROMPT_SIM_BAND){long v=strtol(input,NULL,10);if(v>=POS_MIN&&v<=POS_MAX)dbg_sim_set_position((position_t)v);else line("rejected: position must be 1-5");}
else if(prompt==PROMPT_SIM_FROM){long v=strtol(input,NULL,10);if(v>=POS_MIN&&v<=POS_MAX){sim_from=(position_t)v;line("to position (1-5): ");prompt=PROMPT_SIM_TO;input_len=0;return;}else line("rejected: from position must be 1-5");}
...
static bool handle_prompt_char(char c)
{
    ...
    if (c == '\r' || c == '\n') {
        ...
        if (echo_enabled) console_debug_write("\r\n");
        finish_prompt();
        return true;
    }
    previous_was_cr = false;
    if (c == '\b' || c == 127) {
        if (input_len != 0u) {
            --input_len;
            if (echo_enabled) console_debug_write("\b \b");
        }
        return true;
    }
    if (input_len < CONSOLE_LINE_MAX) input[input_len++] = c;
    if (echo_enabled) {
        char echoed[2] = {c, '\0'};
...
void dbg_adc_trace_dump(void){uint16_t count=controller_adc_trace_snapshot(adc_trace_snapshot,DEBUG_ADC_TRACE_DEPTH);char b[80];snprintf(b,sizeof b,"ADC TRACE count=%u",count);line(b);for(uint16_t i=0;i<count;++i){adc_trace_entry_t *e=&adc_trace_snapshot[i];const char *fault="";if(e->fault){event_kind_t kind=(event_kind_t)(e->fault-1u);fault=kind==EV_FAULT_OVERTRAVEL?" fault=overtravel":kind==EV_FAULT_STALL?" fault=stall":kind==EV_FAULT_DIRECTION?" fault=direction":kind==EV_FAULT_HOME?" fault=home-timeout":" fault=unknown";}snprintf(b,sizeof b,"  tick=%lu adc=%u%s",(unsigned long)e->tick,e->adc,fault);line(b);}}
...
void dbg_position_error(void){char b[112];uint16_t v=encoder_average(),best=UINT16_MAX;position_t nearest=POS_MIN;for(position_t p=POS_MIN;p<=POS_MAX;++p){uint16_t n=encoder_nominal(p),d=v>n?v-n:n-v;if(d<best){best=d;nearest=p;}}int16_t signed_error=(int16_t)v-(int16_t)encoder_nominal(nearest);int32_t degrees_tenths=(int32_t)signed_error*1800/(encoder_nominal(POS_MAX)-encoder_nominal(POS_MIN));snprintf(b,sizeof b,"POSITION ERROR nearest=%u counts=%+d degrees=%+ld.%ld",nearest,signed_error,(long)(degrees_tenths/10),(long)(degrees_tenths<0?-(degrees_tenths%10):degrees_tenths%10));line(b);}
bool dbg_motor_arm(void){line("Manual drive bypasses position limits.\r\nThe mechanism has NO physical end-stops.\r\nType UNCOUPLED to confirm the motor is disconnected:");input_len=0;prompt=PROMPT_ARM;return false;}void dbg_motor_disarm(void){armed=false;(void)post(DBG_OP_EXIT,DIR_STOP,0,0,false);}bool dbg_motor_armed(void){return armed;}void dbg_motor_pulse(direction_t d,uint8_t duty,uint16_t ms){if(!armed){line("debug: not armed");return;}if(!post(DBG_OP_DRIVE,d,duty,ms,false))line("debug: busy");}void dbg_motor_brake(void){if(!post(DBG_OP_BRAKE,DIR_STOP,0,0,false))line("debug: brake request rejected");}void dbg_motor_coast(void){if(post(DBG_OP_COAST,DIR_STOP,0,0,false))line("motor coasting; mechanism may be moved by hand");}void dbg_motor_standby(bool on){(void)post(DBG_OP_STANDBY,DIR_STOP,0,0,on);}void dbg_motor_find_min(direction_t d){char b[48];if(!armed){line("debug: not armed");return;}snprintf(b,sizeof b,"FINDMIN dir=%s",dir_text(d));line(b);action=ACT_FINDMIN_BASE;action_stage=(uint8_t)d;action_duty=DUTY_MIN-DEBUG_FINDMIN_DUTY_OFFSET;action_started=ms_now();action_deadline=action_started+DEBUG_FINDMIN_PULSE_MS;action_min=ADC_MAX_VALUE;action_max=0;}
bool dbg_coupled_confirm(void){line("This test moves the mechanism under closed-loop control.\r\nPosition limits ARE enforced. The mechanism must be connected.\r\nType COUPLED to confirm:");input_len=0;prompt=PROMPT_COUPLED;return false;}void dbg_coupled_clear(void){coupled=false;}bool dbg_coupled(void){return coupled;}
...
void dbg_cal_step_time(void){if(controller_state()==ST_FAULT){line("rejected: motion is blocked while faulted");return;}if(!coupled){dbg_coupled_confirm();return;}if(controller_position()!=3){line("CAL STEP requires position 3");return;}line("CAL STEP");action=ACT_CAL_STEP;action_stage=0;action_worst=0;action_started=0;}
void dbg_cal_travel_time(void){if(controller_state()==ST_FAULT){line("rejected: motion is blocked while faulted");return;}if(!coupled){dbg_coupled_confirm();return;}if(controller_position()!=POS_MAX){line("travel calibration requires position 5");return;}line("CAL TRAVEL");action=ACT_CAL_TRAVEL;action_stage=0;action_started=0;}
void dbg_cal_overshoot(void){if(controller_state()==ST_FAULT){line("rejected: motion is blocked while faulted");return;}if(!coupled){dbg_coupled_confirm();return;}if(controller_position()!=2&&controller_position()!=4){line("CAL OVERSHOOT requires position 2 or 4");return;}{char b[64];snprintf(b,sizeof b,"CAL OVERSHOOT target=3 nominal=%u",DEBUG_NOMINAL_P3);line(b);}action=ACT_CAL_OVER;action_stage=0;action_started=0;action_count=0;action_source=controller_position();action_saved_duty=cfg.duty_approach;action_duty=DEBUG_OVERSHOOT_DUTY_HIGH;}
void dbg_cal_report(void){dbg_cfg_export();}
void dbg_cfg_list(void){dbg_cfg_export();}
bool dbg_cfg_set(const char *k,int32_t v){
    if(v<0)return false;
    if((!strncmp(k,"DUTY_",5)&&v>PWM_WRAP)||(!strncmp(k,"POS_",4)&&v>(int32_t)ADC_MAX_VALUE)||(!strncmp(k,"ADC_SAFE_",9)&&v>(int32_t)ADC_MAX_VALUE)||((strcmp(k,"TIMEOUT_STEP_MS")&&strcmp(k,"TIMEOUT_HOME_MS"))&&v>UINT16_MAX))return false;
    ...
    else if(!strcmp(k,"POS_1_ADC"))n.pos_1_adc=v; else if(!strcmp(k,"POS_2_ADC"))n.pos_2_adc=v; else if(!strcmp(k,"POS_3_ADC"))n.pos_3_adc=v; else if(!strcmp(k,"POS_4_ADC"))n.pos_4_adc=v; else if(!strcmp(k,"POS_5_ADC"))n.pos_5_adc=v;
    else if(!strcmp(k,"POS_WINDOW"))n.pos_window=v; else if(!strcmp(k,"APPROACH_COUNTS"))n.approach_counts=v; else if(!strcmp(k,"ADC_SAFE_MIN"))n.adc_safe_min=v; else if(!strcmp(k,"ADC_SAFE_MAX"))n.adc_safe_max=v; else if(!strcmp(k,"STALL_DELTA"))n.stall_delta=v; else if(!strcmp(k,"STALL_WINDOW_MS"))n.stall_window_ms=v; else if(!strcmp(k,"REVERSE_DELTA"))n.reverse_delta=v;
    else if(!strcmp(k,"DEBOUNCE_MS"))n.debounce_ms=v; else if(!strcmp(k,"BRAKE_HOLD_MS"))n.brake_hold_ms=v; else if(!strcmp(k,"TIMEOUT_STEP_MS"))n.timeout_step_ms=v; else if(!strcmp(k,"TIMEOUT_HOME_MS"))n.timeout_home_ms=v; else return false;
    if(n.duty_min>n.duty_creep||n.duty_creep>n.duty_approach||n.duty_approach>n.duty_normal)return false;
    if(!(n.pos_1_adc<n.pos_2_adc&&n.pos_2_adc<n.pos_3_adc&&n.pos_3_adc<n.pos_4_adc&&n.pos_4_adc<n.pos_5_adc))return false;
    uint16_t gap=n.pos_2_adc-n.pos_1_adc,d=n.pos_3_adc-n.pos_2_adc;if(d<gap)gap=d;d=n.pos_4_adc-n.pos_3_adc;if(d<gap)gap=d;d=n.pos_5_adc-n.pos_4_adc;if(d<gap)gap=d;
    if((uint32_t)n.pos_window*4u>=gap||n.adc_safe_min>=n.adc_safe_max||n.stall_window_ms==0u)return false;
    if(n.debounce_ms==0||n.brake_hold_ms==0||n.timeout_step_ms<DEBUG_CFG_TIMEOUT_MIN_MS||n.timeout_home_ms<DEBUG_CFG_TIMEOUT_MIN_MS)return false;
...
OUT("DUTY_NORMAL",cfg.duty_normal);OUT("DUTY_APPROACH",cfg.duty_approach);OUT("DUTY_CREEP",cfg.duty_creep);OUT("DUTY_MIN",cfg.duty_min);OUT("POS_1_ADC",cfg.pos_1_adc);OUT("POS_2_ADC",cfg.pos_2_adc);OUT("POS_3_ADC",cfg.pos_3_adc);OUT("POS_4_ADC",cfg.pos_4_adc);OUT("POS_5_ADC",cfg.pos_5_adc);OUT("POS_WINDOW",cfg.pos_window);OUT("APPROACH_COUNTS",cfg.approach_counts);OUT("ADC_SAFE_MIN",cfg.adc_safe_min);OUT("ADC_SAFE_MAX",cfg.adc_safe_max);OUT("STALL_DELTA",cfg.stall_delta);OUT("STALL_WINDOW_MS",cfg.stall_window_ms);OUT("REVERSE_DELTA",cfg.reverse_delta);OUT("DEBOUNCE_MS",cfg.debounce_ms);OUT("BRAKE_HOLD_MS",cfg.brake_hold_ms);OUT("TIMEOUT_STEP_MS",cfg.timeout_step_ms);OUT("TIMEOUT_HOME_MS",cfg.timeout_home_ms);
#undef OUT
}
void dbg_fault_show(void){fault_record_t f;char b[160];controller_fault_get(&f);snprintf(b,sizeof b,"FAULT kind=%u ms=%lu state=%s pos=%u target=%u deadline=%lu",f.kind,(unsigned long)f.ms,state_text(f.state),f.pos,f.target,(unsigned long)f.deadline_ms);line(b);}void dbg_history_dump(void){line("HISTORY (newest last)");for(uint8_t i=0;i<controller_history_count();++i){hist_entry_t h;char b[64];if(controller_history_get(i,&h)){const char *k=h.kind==0?"PASS":h.kind==1?"ARR":"UNKNOWN";snprintf(b,sizeof b,"  %lu  %s %u",(unsigned long)h.ms,k,h.pos);line(b);}}}void dbg_counters_show(void){dbg_counters_t c;char b[200];controller_counters_get(&c);snprintf(b,sizeof b,"COUNTERS moves_ok=%lu moves_timeout=%lu faults=%lu limit_rejects=%lu pass_events=%lu tick_overruns=%lu",(unsigned long)c.moves_ok,(unsigned long)c.moves_timeout,(unsigned long)c.faults,(unsigned long)c.limit_rejects,(unsigned long)c.pass_events,(unsigned long)c.tick_overruns);line(b);}void dbg_counters_reset(void){controller_counters_reset();line("counters reset");}void dbg_fault_clear(void){line("Clearing a fault does not restore position.\r\nA home sequence will still be required before any move.\r\nType CLEAR to confirm:");input_len=0;prompt=PROMPT_CLEAR;}
bool dbg_selftest_static(void){tick_stats_t s;controller_timing_get(&s);self_tick_start=s.count;self_all_full=console_event_queue_full();action=ACT_STATIC;action_stage=0;action_count=0;action_deadline=ms_now();line("SELFTEST STATIC");return true;}bool dbg_selftest_motion(void){if(controller_state()==ST_FAULT){line("rejected: motion is blocked while faulted");return false;}if(!coupled){dbg_coupled_confirm();return false;}line("SELFTEST MOTION");action=ACT_SELFTEST;action_stage=0;action_started=0;return true;}

static uint16_t sim_nominal(position_t pos)
{
    return encoder_nominal(pos);
}
...
void dbg_bench_motion_checks(void){motion_check_status_t s;char b[128];controller_motion_checks_get(&s);snprintf(b,sizeof b,"MOTION CHECKS delta=%u window_remaining=%lu ms stall=%s direction=%s",s.current_delta,(unsigned long)s.window_remaining_ms,s.stall_armed?"ARMED":"DISARMED",s.direction_armed?"ARMED":"DISARMED");line(b);}
...
void dbg_sim_set_position(position_t pos){char b[96];if(!encoder_sim_active()){line("rejected: enable simulation first");return;}encoder_sim_set(sim_nominal(pos));snprintf(b,sizeof b,"SIM POSITION %u, adc %u",pos,sim_nominal(pos));line(b);}
void dbg_sim_travel(position_t from,position_t to,uint16_t ms){char b[96];if(!encoder_sim_active()){line("rejected: enable simulation first");return;}if(from==to){line("rejected: from and to must differ");return;}sim_from=from;sim_to=to;sim_band_ms=ms;encoder_sim_set(sim_nominal(from));snprintf(b,sizeof b,"SIM TRAVEL %u -> %u, %u ms/position",from,to,ms);line(b);action=ACT_SIM_TRAVEL;action_stage=0;action_started=ms_now();action_deadline=action_started+DEBUG_SIM_SWEEP_SETTLE_MS;action_count=0;action_pass=true;}
void dbg_sim_park(void){if(!encoder_sim_active()){line("rejected: enable simulation first");return;}line("SIM PARK BETWEEN STATIONS");encoder_sim_set((encoder_nominal(2)+encoder_nominal(3))/2u);action=ACT_SIM_PARK;action_started=ms_now();action_deadline=action_started+DEBUG_SIM_SWEEP_SETTLE_MS;action_stage=0;}
void dbg_sim_overtravel(position_t limit){char b[64];if(!encoder_sim_active()){line("rejected: enable simulation first");return;}snprintf(b,sizeof b,"SIM OVERTRAVEL LIMIT %u",limit);line(b);sim_from=limit;encoder_sim_set(sim_nominal(limit));action=ACT_SIM_DRIFT;action_stage=0;action_deadline=ms_now()+DEBUG_SIM_SWEEP_SETTLE_MS;}
void dbg_sim_sweep(void){if(!encoder_sim_active()){line("rejected: enable simulation first");return;}(void)post(DBG_OP_ENTER,DIR_STOP,0,0,false);line("SIM SWEEP WINDOWS");encoder_sim_set(0);action=ACT_SIM_SWEEP;action_count=0;action_last_pos=POS_UNKNOWN;action_pass=true;action_deadline=ms_now()+DEBUG_SIM_SWEEP_SETTLE_MS;}
...
static void action_poll(uint32_t now)
{
    ...
    if (action == ACT_SIM_PARK) {
        if((int32_t)(now-action_deadline)<0)return;
        line(encoder_confirmed()==POS_BETWEEN&&controller_state()==ST_IDLE?"  RESULT: PASS  (known between-station angle, controller idle)":"  RESULT: FAIL  (between-station classification or state incorrect)");action=ACT_NONE;return;
    }
    if (action == ACT_SIM_DRIFT) {
        if(action_stage==0u){if((int32_t)(now-action_deadline)<0||encoder_confirmed()!=sim_from)return;uint16_t injected=sim_from==POS_MIN?CFG_ADC_SAFE_MIN-1u:CFG_ADC_SAFE_MAX+1u;snprintf(b,sizeof b,"  established pos %u (adc %u)",sim_from,sim_nominal(sim_from));line(b);encoder_sim_set(injected);snprintf(b,sizeof b,"  injected adc %u beyond safe range",injected);line(b);action_stage=1u;action_started=now;return;}if(controller_state()!=ST_FAULT)return;snprintf(b,sizeof b,"  fault after %lu ms, dir %s, STBY %u",(unsigned long)(now-action_started),dir_text(motor_direction()),gpio_get(PIN_STBY));line(b);line(motor_direction()==DIR_STOP&&!gpio_get(PIN_STBY)?"  RESULT: PASS  (braked, disabled, faulted)":"  RESULT: FAIL  (unsafe terminal state)");action=ACT_NONE;return;
    }
    if (action == ACT_SIM_SWEEP) {
        if((int32_t)(now-action_deadline)<0)return;
        uint16_t value=(uint16_t)action_count;position_t expected=POS_BETWEEN;if(value<CFG_ADC_SAFE_MIN||value>CFG_ADC_SAFE_MAX)expected=POS_UNKNOWN;else for(position_t p=POS_MIN;p<=POS_MAX;++p){uint16_t n=encoder_nominal(p),d=value>n?value-n:n-value;if(d<=CFG_POS_WINDOW){expected=p;break;}}if(encoder_confirmed()!=expected)action_pass=false;if(action_count==ADC_MAX_VALUE){snprintf(b,sizeof b,"  safe range %u..%u",CFG_ADC_SAFE_MIN,CFG_ADC_SAFE_MAX);line(b);for(position_t p=POS_MIN;p<=POS_MAX;++p){uint16_t n=encoder_nominal(p);snprintf(b,sizeof b,"  position %u window %u..%u",p,n-CFG_POS_WINDOW,n+CFG_POS_WINDOW);line(b);}line(action_pass?"  RESULT: PASS  (0..4095 matches windows and safe range)":"  RESULT: FAIL  (classification mismatch)");(void)post(DBG_OP_SIM_ENABLE,DIR_STOP,0,0,true);action=ACT_NONE;return;}++action_count;encoder_sim_set((uint16_t)action_count);action_deadline=now+DEBUG_SIM_SWEEP_SETTLE_MS;return;
    }
    if (action == ACT_STATIC) {
        ...
        bool ordered = CFG_POS_1_ADC < CFG_POS_2_ADC && CFG_POS_2_ADC < CFG_POS_3_ADC && CFG_POS_3_ADC < CFG_POS_4_ADC && CFG_POS_4_ADC < CFG_POS_5_ADC;
        uint16_t gap=CFG_POS_2_ADC-CFG_POS_1_ADC,d=CFG_POS_3_ADC-CFG_POS_2_ADC;if(d<gap)gap=d;d=CFG_POS_4_ADC-CFG_POS_3_ADC;if(d<gap)gap=d;d=CFG_POS_5_ADC-CFG_POS_4_ADC;if(d<gap)gap=d;
        ...
        bool position_ok = pos == POS_UNKNOWN || pos == POS_BETWEEN || (pos >= POS_MIN && pos <= POS_MAX);
        snprintf(b,sizeof b,"  ADC responds %s",adc_ok?"PASS":"FAIL");line(b);
        snprintf(b,sizeof b,"  positions ordered %s  windows %s",ordered?"PASS":"FAIL",windows?"PASS":"FAIL");line(b);
        ...
        line(adc_ok&&ordered&&windows&&duties&&timeouts&&tick_ok&&queue_ok&&position_ok?"SELFTEST STATIC PASS":"SELFTEST STATIC FAIL");
        action = ACT_NONE;
        return;
    }
    ...
    if (action == ACT_CAL_POS_SAMPLE) {
        ...
        else { cfg.pos_1_adc=action_means[0];cfg.pos_2_adc=action_means[1];cfg.pos_3_adc=action_means[2];cfg.pos_4_adc=action_means[3];cfg.pos_5_adc=action_means[4];line("CALIBRATED POSITION VALUES");dbg_position_table();for(uint8_t i=1;i<POS_MAX;++i)if((uint16_t)(action_means[i]-action_means[i-1u])<DEBUG_CAL_MIN_SEPARATION)line("  MARGINAL adjacent separation under 200");action=ACT_NONE; }
        return;
    }
