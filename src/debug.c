...
static void adc_trace_dump_poll(void)
{
    ...
    if (adc_trace_dump_phase == 0u) {
        ...
        if (!controller_adc_trace_get(adc_trace_dump_index, &entry)) {
            ...
            const char *fault = "";
            if (entry.fault) {
                event_kind_t kind = (event_kind_t)(entry.fault - 1u);
                fault = kind == EV_FAULT_OVERTRAVEL ? " fault=overtravel" :
                    kind == EV_FAULT_STALL ? " fault=stall" :
