...
bool dbg_cfg_set(const char *k,int32_t v){
    if(v<0)return false;
    if((!strncmp(k,"DUTY_",5)&&v>PWM_WRAP)||(!strncmp(k,"POS_",4)&&v>(int32_t)ADC_MAX_VALUE)||(!strncmp(k,"ADC_SAFE_",9)&&v>(int32_t)ADC_MAX_VALUE)||v>UINT16_MAX)return false;
    ...
    if(n.pos_window>=gap/4u||n.adc_safe_min>=n.adc_safe_max||n.stall_window_ms==0u)return false;
